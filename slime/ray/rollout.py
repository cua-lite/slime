...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        # Lazy multimodal payloads, materialized once per RL iter on the train
        # side by ``slime.utils.data.materialize_lazy_payloads`` (called from
        # ``actor._get_rollout_data`` before any DataIterator slices micro-batches).
        # Mutually exclusive with multimodal_train_inputs per-Sample —
        # enforcement lives in materialize_lazy_payloads.
        if any(sample.multimodal_lazy_payloads is not None for sample in samples):
            train_data["multimodal_lazy_payloads"] = [sample.multimodal_lazy_payloads for sample in samples]
