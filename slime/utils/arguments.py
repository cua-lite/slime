...
def get_slime_extra_args_provider(add_custom_arguments=None):
    def add_slime_arguments(parser):
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                "--multimodal-lazy-expand-fn-path",
                type=str,
                default=None,
                help=(
                    "Dotted path to a function that materializes "
                    "Sample.multimodal_lazy_payloads into the standard "
                    "multimodal_train_inputs dict. Called once per RL iter "
                    "from actor._get_rollout_data via "
                    "slime.utils.data.materialize_lazy_payloads, before any "
                    "phase (log_probs / train) builds a DataIterator. "
                    "Signature: fn(payload: dict, args=args, **_) -> dict. "
                    "``args`` is the slime arg namespace; expand hooks that need "
                    "model-side state (e.g. an HF processor for encoded-bytes "
                    "payloads) read it from ``args.hf_checkpoint``. Required "
                    "when any Sample carries multimodal_lazy_payloads; ignored "
                    "otherwise."
                ),
            )
